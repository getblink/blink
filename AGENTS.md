...
- When validating `tldr_app/` locally, use `bash tldr_app/scripts/install_local_app.sh` and launch only `~/Applications/TLDR.app`; the installer resets TCC on every rebuild by default so permissions attach to the fresh binary. Keep the root `./tldr` harness as a scratchpad/dev loop, not the shipped app surface.
- Cutting a public Sparkle release goes through `tldr_app/scripts/release.sh`. Bump `tldr_app/project.yml`'s `CFBundleShortVersionString` (xcodegen overwrites `Info.plist` from `project.yml`, so editing `Info.plist` directly is wiped). Export the repo-root `.env` (`set -a && source .env && set +a`) before invoking the script, and verify the build log contains `[tldr] stamping SUFeedURL=...` and `[tldr] stamping SUPublicEDKey ...` — a missing line means the new build cannot discover or verify future updates and must not be uploaded. Full env-var list and known gotchas (Sparkle `sign_update` chicken-and-egg on first build, `TLDR_SIGN_IDENTITY` pin for duplicate Developer ID certs) live in `tldr_app/README.md` → Sparkle Releases.
