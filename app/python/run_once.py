...
DEFAULT_PROMPT = (
    ...
    "When focused_field.text_before_caret and focused_field.text_after_caret\n"
    "are present, they are the text immediately before and after the user's\n"
    "caret in the focused field. Both are already in the field; insert your\n"
    "output between them and do not repeat them. If text_selected is present,\n"
    "treat it as the span the user wants replaced. If text_before_caret_line\n"
    "and text_after_caret_line are present, use them the same way at line\n"
    "granularity.\n\n"
    "Examples:\n\n"
...
TARGET_OCR_PACKET_FULL_IMAGE_FALLBACK_MODES = {
    "source_packet_target_ocr_packet",
    "source_packet_target_ocr_or_full_image",
    NATIVE_SOURCE_OCR_REQUEST_MODE,
}
...
def _role_supports_vision(runtime_selection: dict[str, Any], role: str) -> bool:
    ...
    supports = preset.get("supports_vision")
    result = supports if isinstance(supports, bool) else True
    model = section.get("model")
    overrides = preset.get("model_overrides")
    if isinstance(model, str) and isinstance(overrides, dict):
        model_override = overrides.get(model)
        if (
            isinstance(model_override, dict)
            and isinstance(model_override.get("supports_vision"), bool)
        ):
            result = bool(model_override["supports_vision"])
    return result


def _full_target_image_role(runtime_selection: dict[str, Any]) -> str:
    if _role_supports_vision(runtime_selection, RUNTIME_ROLE_PASTE):
        return RUNTIME_ROLE_PASTE
    if _role_supports_vision(runtime_selection, RUNTIME_ROLE_EXTRACTOR):
        return RUNTIME_ROLE_EXTRACTOR
    return RUNTIME_ROLE_PASTE
...
def _should_use_full_target_image(
    request_mode: str,
    target_packet_payload: dict[str, Any],
) -> bool:
    if request_mode not in TARGET_OCR_PACKET_FULL_IMAGE_FALLBACK_MODES:
        return False
    return (
        target_packet_payload.get("completeness") == "needs_target_image"
        or bool(target_packet_payload.get("fallback_reasons"))
    )
...
def _prepared_source_matches(
    ...
    if request_mode == NATIVE_SOURCE_OCR_REQUEST_MODE:
        return (
            ...
            and prepared_source.get("source_packet_kind") == NATIVE_SOURCE_PACKET_KIND
...
def _build_baseline_instruction(
...
def main(argv: list[str] | None = None) -> int:
    ...
    _save_json(
        bundle_dir / "target_metadata.prompt.json",
        compact_target_metadata(target_metadata, caret=caret),
    )
    ...
    if args.skip_gemini:
        ...
        try:
            if request_mode == "baseline_full_images":
                instruction_text = _build_baseline_instruction(target_metadata, caret=caret)
                ...
                if _prepared_source_matches(prepared_source, runtime_selection, source_extract_prompt_path):
                    ...
                    if request_mode == NATIVE_SOURCE_OCR_REQUEST_MODE:
                        source_packet_result = build_native_ocr_source_packet(source_path=source_dst)
                        source_packet_payload = {
                            "status": source_packet_result["status"],
                            "source_packet_kind": source_packet_result["source_packet_kind"],
                            "packet_text": source_packet_result["packet_text"],
                            "build_log": source_packet_result["build_log"],
                            "runtime_signature": {
                                "request_mode": request_mode,
                                "source_packet_kind": source_packet_result["source_packet_kind"],
                ...
                source_packet_kind = str(source_packet_payload.get("source_packet_kind") or "unknown")
                ...
                if request_mode == "source_packet_full_target_image":
                    ...
                    full_target_role = _full_target_image_role(runtime_selection)
                    full_target_settings = (
                        extractor_settings
                        if full_target_role == RUNTIME_ROLE_EXTRACTOR
                        else paste_settings
                    )
                    run_log["target_context"] = {
                        "mode": "full_target_image",
                        "full_target_image_role": full_target_role,
                    }
                    generation_result = run_source_packet_target_full_image(
                        settings=full_target_settings,
                        prompt_text=generation_prompt_text,
                        source_packet_text=source_packet_text,
                        source_packet_kind=source_packet_kind,
                        target_path=target_dst,
                        target_metadata=target_metadata,
                        runtime=get_runtime(full_target_role),
                        caret=caret,
                    )
                    if full_target_role != RUNTIME_ROLE_PASTE:
                        run_log.setdefault("warnings", []).append(
                            "full_target_image_used_extractor_runtime"
                        )
                else:
                    ...
                    run_log["target_context"] = {
                        "mode": "target_ocr_packet",
                        "completeness": target_packet_payload.get("completeness"),
                        "fallback_reasons": target_packet_payload.get("fallback_reasons"),
                        "focused_label_hint": target_packet_payload.get("focused_label_hint"),
                        "packet_chars": target_packet_payload.get("packet_chars"),
                    }

                    use_full_target_image = _should_use_full_target_image(
                        request_mode,
                    ...
                    if use_full_target_image:
                        ...
                        run_log["target_context"]["mode"] = "full_target_image"
                        run_log["target_context"]["full_target_image_role"] = full_target_role
                        generation_result = run_source_packet_target_full_image(
                            settings=full_target_settings,
                            prompt_text=generation_prompt_text,
                            source_packet_text=source_packet_text,
                ...
                run_log["source_packet"] = {
                    ...
                    "prepared": _prepared_source_matches(prepared_source, runtime_selection, source_extract_prompt_path),
