...
`app/` is the shipped macOS surface for the TL;DR + reply suggestions
experiment. The older intelligent copy-paste tester app is archived under
`experiments/blink-copy-paste/` and kept only for fixture replay.

Blink is a menubar app:

- `Ctrl+Opt+Space` (default) captures the frontmost window with ScreenCaptureKit;
  the hotkey is configurable via `~/.blink/settings.json`. A single press
...
- The bundled Python runtime calls the Blink server when `BLINK_PROXY_URL` +
  `BLINK_PROXY_TOKEN` are configured, and otherwise falls back to direct Gemini.
  `BLINK_DISABLE_PROXY=1` forces direct-Gemini regardless (used when iterating
...
- Config, credentials, and prompt overrides: `~/.blink/`
- Run artifacts: `~/Library/Application Support/Blink/runs/`
- Pending crash-recovery records: `~/Library/Application Support/Blink/pending/`
- Local install: `~/Applications/Blink.app`

Stateful POC:

- Before each request, `blink_once.py` scans recent local run artifacts.
...
```bash
bash app/scripts/fetch_python.sh
BLINK_SKIP_TCC_RESET=1 bash app/scripts/build.sh
bash app/scripts/install_local_app.sh
bash app/scripts/make_dmg.sh
```

The DMG lands at `app/build/Blink-0.1.0.dmg`.

Use `install_local_app.sh` for dogfood rebuilds. It resets TCC by default after
installing `~/Applications/Blink.app`; pass `--skip-tcc-reset` only when you are
...
For Gemini or proxy-backed runs, put runtime env in `~/.blink/.env`:
...
Gate 2 renamed shipped-app variables from `TLDR_*` to `BLINK_*`. Scripts and
runtime code still accept the old names for one release and print a deprecation
warning when they are used. Update local `.env` files to the new names before
the next release removes the aliases.

| Old | New |
| --- | --- |
| `TLDR_DEVELOPMENT_TEAM` | `BLINK_DEVELOPMENT_TEAM` |
| `TLDR_SPARKLE_FEED_URL` | `BLINK_SPARKLE_FEED_URL` |
| `TLDR_SPARKLE_PUBLIC_ED_KEY` | `BLINK_SPARKLE_PUBLIC_ED_KEY` |
| `TLDR_SIGN_IDENTITY` | `BLINK_SIGN_IDENTITY` |
| `TLDR_DISABLE_PROXY` | `BLINK_DISABLE_PROXY` |
| `TLDR_PROXY_URL`, `TLDR_PROXY_TOKEN` | `BLINK_PROXY_URL`, `BLINK_PROXY_TOKEN` |
| `TLDR_R2_*` | `BLINK_R2_*` |
...
Create `app/scripts/config.env` locally; it is gitignored:
...
notarizes `Blink.app`, packages the DMG, then signs, notarizes, staples, and
...
`~/.blink/.env`. Treat the token as compromised after shipping; cap it with the
server-side per-token rate limit (`BLINK_TOKEN_RATE_LIMIT_PER_MINUTE`, backed by
`REDIS_URL` when available) and revoke it in `BLINK_API_TOKENS` when needed.

Optional overrides:

- `~/.blink/settings.json`
- `~/.blink/prompts/prompt.txt`
- `~/.blink/runtime-config.json`

## Sparkle Releases

Blink uses Sparkle 2 for prompted updates. The release flow lives in
`app/scripts/release.sh`: it fetches the pinned Python runtime, builds,
signs, notarizes, packages the DMG, signs the update for Sparkle, writes
`app/build/appcast.xml`, and uploads the DMG + appcast to R2. Set
`BLINK_RELEASE_UPLOAD=0` for a local dry run.
...
3. **First run in a fresh workspace also needs `BLINK_SPARKLE_SIGN_UPDATE`.**
   ...
   ```bash
   export BLINK_SPARKLE_SIGN_UPDATE=$(ls /Users/$USER/conductor/workspaces/blink/*/app/build/DerivedData/SourcePackages/artifacts/sparkle/Sparkle/bin/sign_update 2>/dev/null | head -1)
   ```

   or pre-resolve dependencies once (`xcodebuild -resolvePackageDependencies
   -project app/Blink.xcodeproj`). Subsequent releases in the same
...
BLINK_TEAM_ID=8W5FCW3BXK              # team ID for codesigning fallback
BLINK_SIGN_IDENTITY=<sha-1>            # required if the keychain has multiple
...
BLINK_NOTARY_PROFILE=Blink-NOTARY      # default; set up once with
                                      #   xcrun notarytool store-credentials Blink-NOTARY

# Sparkle
BLINK_SPARKLE_FEED_URL=https://<r2-public-domain>/appcast.xml
BLINK_SPARKLE_PUBLIC_ED_KEY=<base64 from Sparkle generate_keys>
# Optional: BLINK_SPARKLE_KEYCHAIN_ACCOUNT (defaults to Sparkle's lookup)
# Optional: BLINK_SPARKLE_SIGN_UPDATE (see "First run" note above)

# Cloudflare R2
BLINK_R2_BUCKET=blink-releases
BLINK_R2_PUBLIC_DOMAIN=<pub-*.r2.dev or your custom domain>
BLINK_R2_ENDPOINT=https://<account-id>.r2.cloudflarestorage.com
BLINK_R2_ACCESS_KEY_ID=<S3-compatible token id>
...
build log shows both `[blink] stamping SUFeedURL=...` and `[blink] stamping
SUPublicEDKey from BLINK_SPARKLE_PUBLIC_ED_KEY` lines before uploading.
