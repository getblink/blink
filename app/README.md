...
app/
  ...
  Blink/                          ← Swift sources
    ...
    SourceTextCapture.swift       exact source text via AX selection or Cmd+C restore
  ...
  Resources/                      ← copied into .app/Contents/Resources at build
    ...
    target_text_only_prompt.txt   hybrid text-only target prompt
...
- `runtime-config.json` — extractor/paste provider presets, models, and request mode
...
- request-mode toggles (`baseline`, `source packet + full target`, `source packet + target OCR packet`, `source packet + OCR fallback`, `experimental source text/OCR + text target`)
...
The hybrid experimental mode first tries exact selected/copied source text, preserving paragraph and multiline formatting, falls back to deterministic native OCR paragraphs for image-like sources, then routes target-time inference to a text-only call for strong text-field AX roles and falls back to the full target image otherwise.
...
`host_profile.json`,
`runtime_selection.json`, `geometry.json`, `source_text.json`, `source_packet.txt`,
`target_ocr_packet.txt`, `target_ocr_text.txt`, `generation.prompt.txt`, and
