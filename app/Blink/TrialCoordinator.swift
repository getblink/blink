...
final class TrialCoordinator {
    ...
    func setSource() {
        let runtime = runtimeStore.currentSnapshot()
        queue.async { [self] in
            ...
            do {
                ...
                var sourceTextPayload: [String: Any]? = nil
                if runtime.requestMode == .sourceOCRTargetTextOrFullImage {
                    let sourceTextStartedAt = Date()
                    let sourceTextStartedPerf = monotonicNow()
                    status("reading selected source text…")
                    sourceTextPayload = SourceTextCapture.capture()
                    let sourceTextFinishedAt = Date()
                    let sourceTextMS = durationMS(since: sourceTextStartedPerf)
                    sourceHostProfile["source_text_capture_started_at"] = isoString(sourceTextStartedAt)
                    sourceHostProfile["source_text_capture_finished_at"] = isoString(sourceTextFinishedAt)
                    sourceHostProfile["source_text_capture_ms"] = sourceTextMS
                    sourceHostProfile["source_text_status"] = sourceTextPayload?["status"] as Any
                    sourceHostProfile["source_text_method"] = sourceTextPayload?["method"] as Any
                    sourceHostProfile["source_text_chars"] = sourceTextPayload?["text_chars"] as Any
                    sourceTimingSummary["host_source_text_capture_ms"] = sourceTextMS
                }
    ...
    func runTarget() {
        ...
        queue.async { [self] in
            ...
            do {
                ...
                if let sourceText = source.sourceText {
                    try ArtifactWriter.writeJSON(
                        sourceText,
                        to: tempDir.appendingPathComponent("source_text.json")
                    )
                }
            } catch {
            ...
            PythonRunner.runOnce(
                ...
                sourceTextJSON: source.sourceText == nil
                    ? nil
                    : tempDir.appendingPathComponent("source_text.json"),
    ...
    private func maybePrepareSource(
        ...
        let sourceTextURL = tempDir.appendingPathComponent("source_text.json")
        try capture.pngData.write(to: sourceURL)
        try ArtifactWriter.writeJSON(runtime.payload, to: runtimeURL)
        if let sourceText {
            try ArtifactWriter.writeJSON(sourceText, to: sourceTextURL)
        }
        ...
        return try PythonRunner.prepareSourceSync(
            ...
            sourceTextJSON: sourceText == nil ? nil : sourceTextURL,
            settingsJSON: runtime.settingsPath
        )
    }
