...
/// Builds the style-knobs UI as a reusable `NSView`, so both the standalone
/// Style window and the Settings ▸ Style pane render the same layout.
/// Owns the segmented-control state, the "About me" `NSTextView` delegate,
/// and the `runtimeStore.$style` subscription.
@MainActor
final class StylePaneController: NSObject, NSTextViewDelegate {
    ...
    init(runtimeStore: RuntimeConfigStore, contentWidth: CGFloat = 460 - 48) {
        self.runtimeStore = runtimeStore
        self.contentView = NSView()
        super.init()
        buildContent(width: contentWidth)
        applyCurrentStyle()
    }
    ...
    private func buildContent(width: CGFloat) {
        ...
        for (index, spec) in Self.knobs.enumerated() {
            ...
            NSLayoutConstraint.activate([
                segmented.widthAnchor.constraint(equalToConstant: width),
            ])
        }
        ...
        defaultsBtn.toolTip = "Reset all knobs to balanced defaults."
        proBtn.toolTip = "Formal tone preset."
        boldBtn.toolTip = "Agentic, direct, terse preset."
        resetBtn.toolTip = "Restore knobs without clearing About me."
        ...
        NSLayoutConstraint.activate([
            ...
            buttonRow.widthAnchor.constraint(equalToConstant: width),
        ])
    }
    ...
    @objc func applyDefaultPreset() { runtimeStore.style = .default }

    @objc func applyProfessionalPreset() {
        var style = runtimeStore.style
