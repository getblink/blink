#!/usr/bin/env bash
# Fetch a self-contained python-build-standalone runtime into tldr_app/python-dist/.
...
case "$PIN_KEY" in
    "3.11.9|20240415|aarch64|apple-darwin-install_only")
        ...
        ;;
    *)
        echo "[tldr] error: no pinned SHA256 for python-build-standalone tuple: $PIN_KEY" >&2
        echo "[tldr] update this script from the upstream release asset before downloading:" >&2
        echo "[tldr] https://github.com/indygreg/python-build-standalone/releases/tag/$PBS_RELEASE" >&2
        exit 1
        ;;
esac

echo "[tldr] fetching $TARBALL"
echo "[tldr] from   $URL"
...
echo "[tldr] verifying SHA256"
echo "$EXPECTED_SHA256  $TMP_DIR/$TARBALL" | shasum -a 256 -c -

echo "[tldr] extracting → $DIST_DIR"
rm -rf "$DIST_DIR"
mkdir -p "$DIST_DIR"
...
if [[ ! -x "$PYTHON_BIN" ]]; then
    echo "[tldr] error: expected python3 at $PYTHON_BIN" >&2
    exit 1
fi

echo "[tldr] installing google-genai into the dist"
