            ...
            with conn.cursor() as cur:
                ...
                if cursor:
                    ...
                    if cursor_row is None:
                        return ([], None)
                    query = """
                        SELECT id, email_original, source, created_at, ip_hash, user_agent
                        FROM beta_signups
                        WHERE (created_at, id) < (%s, %s)
                        ORDER BY created_at DESC, id DESC
                        LIMIT %s
                    """
                    params = (cursor_row[0], cursor_row[1], clamped_limit + 1)
                else:
                    query = """
                        SELECT id, email_original, source, created_at, ip_hash, user_agent
                        FROM beta_signups
                        ...
                        LIMIT %s
                    """
                    params = (clamped_limit + 1,)
                cur.execute(query, params)
                fetched = cur.fetchall()
        page_rows = fetched[:clamped_limit]
        next_cursor = page_rows[-1][0] if len(fetched) > clamped_limit and page_rows else None
        rows: list[dict[str, Any]] = []
        for row in page_rows:
            item = {
                "id": row[0],
                "email_original": row[1],
                "source": row[2],
                "created_at": row[3].isoformat() if hasattr(row[3], "isoformat") else str(row[3]),
            }
            if verbose:
                item["ip_hash"] = row[4]
                item["user_agent"] = row[5]
            rows.append(item)
