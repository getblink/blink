...
## 2026-04-26 — Clipboard-first source text merged into paste-latency v3

- **Hypothesis:** For paragraph-heavy source pages, exact local source text captured at `⌃⇧C` should beat screenshot OCR for formatting fidelity while preserving the v3 paste-latency work: split extractor/paste runtime config, source packet precompute, warm Python worker, stopwatch timings, and model-compatibility guardrails.
- **Setup:** Ported the dirty `speed-up-paste-latency-v2` Kathmandu work into Oslo surgically. The Swift source path now tries AX selected text first, then a pasteboard-preserving Cmd+C fallback, writes `source_text.json`, and passes it to both `prepare_source.py` and paste-time `run_once.py` including warm-worker JSON requests. Python now supports `--source-text`, builds `local_source_text` packets with digest validation, rejects stale prepared local-text packets, and falls back through the shared native OCR path when local text is empty or invalid. Native source OCR also drops common top toolbar/menu rows, clamps top-band chrome more aggressively, and splits prompt/body false joins. Artifact docs and the in-app inspector now include `source_text.json` and source-text timing.
- **Input type(s):** Hybrid tester app path, especially selectable text sources and paragraph-heavy form/source pages.
- **Target field type(s):** Text-only hybrid target routing remains unchanged; Oslo's split runtime and warm-worker paste path remain authoritative.
- **Outcome:** Implemented and locally validated. `python3 -m unittest app/python/tests/test_hybrid_request_mode.py`, `python3 -m unittest discover app/python/tests`, `python3 -m compileall app/python`, `BLINK_SKIP_TCC_RESET=1 bash app/scripts/build.sh`, and `bash app/scripts/install_local_app.sh --reset-tcc` all pass. `app/python-dist` was missing before the packaged build, so `bash app/scripts/fetch_python.sh` was run first.
- **Evidence / examples:** `app/Blink/SourceTextCapture.swift`, `app/Blink/TrialCoordinator.swift`, `app/Blink/PythonRunner.swift`, `app/python/source_ocr.py`, `app/python/prepare_source.py`, `app/python/run_once.py`, `app/python/tests/test_hybrid_request_mode.py`, `docs/ARTIFACT_SCHEMA.md`.
- **Decision:** Keep exact local source text as the first source-packet strategy only in `source_ocr_target_text_or_full_image`; image-like or failed text captures still fall back to deterministic native OCR without provider credential resolution.
- **Next step:** Dogfood on a paragraph-heavy Google Docs/Edge source. The acceptance bundle should contain `source_text.json`; when selectable text exists, `prepared_source.json.source_packet_kind` should be `local_source_text` and `source_packet.txt` should preserve paragraph breaks. Empty local text should still produce `native_ocr_paragraphs`.
